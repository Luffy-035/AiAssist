...
const Messages = ({ messages, chatbotName }: {
    ...
    const isReviewPage = pathname.includes("review-sessions")
        ...
        <div className='flex-1 flex flex-col overflow-y-auto space-y-10 py-10 px-5 bg-white rounded-lg'>
            {messages.map((message) => {
                ...
                return <div
                    ...
                    {isReviewPage && (
                        <p className='absolute -bottom-5 text-xs text-gray-300'>sent {new Date(message.created_at).toLocaleString('en-US')}</p>
                    )}
                    ...
                    <div className={`chat-bubble break-words text-white ${isSender ? "chat-bubble-primary bg-[#4D7DF8]" : "chat-bubble-secondary bg-gray-200 text-gray-700"}`}>
                        <ReactMarkdown
                            ...
                            components={{
                                ...
                                p:({node,...props}) => (
                                    <p
                                        {...props}
                                        className={`whitespace-break-spaces mb-5 ${message.content === "Thinking..." && "animate-pulse"} ${isSender?"text-white":"text-gray-700"}`}
                                    />
                                ),
                                a:({node,...props}) => (
                                    <a {...props}
