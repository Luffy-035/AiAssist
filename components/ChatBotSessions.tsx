        ...
        <div className='bg-white'>
            <Accordion type='single' collapsible>
                {sortedChatbots.map((chatbot) => {
                    ...
                    return <AccordionItem key={chatbot.id} value={`item-${chatbot.id}`} className='px-10 py-5'>
                        {hasSessions ? (
                            <>
                                <AccordionTrigger>
                                    <div className='flex text-left items-center w-full'>
                                        ...
                                        <div className='flex flex-1 justify-between space-x-4'>
                                            <p className='ml-4'>{chatbot.name}</p>
                                ...
                                <AccordionContent className='space-y-5 p-5 bg-gray-100 rounded-md'>
                                    {chatbot.chat_sessions.map((session) => (
                                        <Link key={session.id} href={`/review-sessions/${session.id}`} className='relative p-10 bg-[#2991EE] text-white rounded-md block'>
                                            ...
                                            <p className='absolute tp[-5 right-5 text-sm'>
