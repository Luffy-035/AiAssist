...
import { GET_CHATBOTS_BY_USER } from '@/lib/queries/queries'
...
const Page = async () => {
  ...
  const sortedChatbotsByUser: Chatbot[] = [...chatbotsByUser].sort((a, b) => {
  ...
  return (
    <div className='flex-1 pb-20 p-10'>
      ...
      {sortedChatbotsByUser.length === 0 && (
        <div>
          <p>You have not created any chatbots yet, Click on the button below to create one.
          </p>
      ...
      <ul className='flex flex-col space-y-5'>
        {sortedChatbotsByUser.map((chatbot) => (
          <Link key={chatbot.id} href={`/edit-chatbot/${chatbot.id}`}>
            <li className='relative p-10 rounded-md border max-w-3xl bg-white'>
